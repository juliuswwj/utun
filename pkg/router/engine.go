...
func (e *Engine) SetLocalIPs(v4, v6 string) {
	e.localVPNIP = v4
	e.localVPNIP6 = v6
}
...
func (e *Engine) SetListener(l *transport.MultiPortListener) {
	e.udp = l
	if l != nil { l.SetHandler(e.ProcessIncoming) }
}
...
func (e *Engine) Start(ctx context.Context) error {
	go e.tunToUDP(ctx)
	if e.udp != nil { go e.periodicRA(ctx) }
	return nil
}
...
func (e *Engine) sendRALAN() {
	...
	pio := icmp[16:]; pio[0] = 3; pio[1] = 4; pio[2] = 64; pio[3] = 0xc0
	binary.BigEndian.PutUint32(pio[4:8], 2592000); binary.BigEndian.PutUint32(pio[8:12], 604800)
	copy(pio[16:32], prefix.IP.To16())
	slla := icmp[48:]; slla[0] = 1; slla[1] = 1; copy(slla[2:8], e.lanMAC)

	ipHeader := make([]byte, 40)
	ipHeader[0] = 6<<4; binary.BigEndian.PutUint16(ipHeader[4:6], uint16(56))
	ipHeader[6] = 58; ipHeader[7] = 255
	copy(ipHeader[8:24], srcLL); copy(ipHeader[24:40], net.ParseIP("ff02::1"))
	csum := e.icmp6Checksum(append(ipHeader, icmp...), srcLL, net.ParseIP("ff02::1"))
	binary.BigEndian.PutUint16(icmp[2:4], csum)

	eth := make([]byte, 14); copy(eth[0:6], []byte{0x33, 0x33, 0x00, 0x00, 0x00, 0x01})
	copy(eth[6:12], e.lanMAC); binary.BigEndian.PutUint16(eth[12:14], 0x86DD)
...
func (e *Engine) sendRA(targetSession *transport.Session) {
	...
	icmp := packet[40:]; icmp[0] = 134; icmp[4] = 64
	binary.BigEndian.PutUint16(icmp[6:8], 1800)
	pio := icmp[16:]; pio[0] = 3; pio[1] = 4; pio[2] = 64; pio[3] = 0xc0
	binary.BigEndian.PutUint32(pio[4:8], 2592000); binary.BigEndian.PutUint32(pio[8:12], 604800)
	...
	binary.BigEndian.PutUint16(icmp[2:4], csum)
	if targetSession != nil { e.sendPacketToSession(packet, targetSession)
	} else {
...
func (e *Engine) icmp6Checksum(packet []byte, src, dst net.IP) uint16 {
	...
	for i := 0; i < len(payload); i += 2 {
		if i+1 < len(payload) { sum += uint32(binary.BigEndian.Uint16(payload[i:i+2]))
		} else { sum += uint32(payload[i]) << 8 }
	}
...
func (e *Engine) sendPacketToSession(packet []byte, session *transport.Session) {
	...
	if err != nil { return }
	if len(session.RemoteAddrs) > 0 { e.udp.WriteTo(sealed, session.RemoteAddrs[0]) }
}
	...
	for {
		...
		default:
			...
			if e.cfg == nil && n >= 40 && (packet[0]>>4) == 6 {
				...
				if e.lanPrefix != nil && e.lanPrefix.Contains(net.ParseIP(srcIP)) { e.ensureKernelRoute(srcIP, e.lanIfName) }
			}
...
func (e *Engine) handleOutbound(packet []byte) {
	...
	if err != nil || len(session.RemoteAddrs) == 0 || session.Cipher == nil { return }
	e.sendPacketToSession(packet, session)
}

func (e *Engine) ProcessIncoming(p transport.IncomingPacket) {
	if p.Raw != nil {
		header := p.Raw[0]
	...
	if !ok || session.Cipher == nil { return }
	e.sm.UpdateActivity(p.SessionID, p.Addr)
	decrypted, err := crypto.Decrypt(session.Cipher, p.Nonce, p.Payload, nil)
	if err != nil { return }

	// STRICT TRIM: Based on IP header total length
	realPacket := decrypted
	if len(decrypted) >= 20 {
		version := decrypted[0] >> 4
		if version == 4 {
			totalLen := int(binary.BigEndian.Uint16(decrypted[2:4]))
			if totalLen > 0 && totalLen <= len(decrypted) { realPacket = decrypted[:totalLen] }
		} else if version == 6 && len(decrypted) >= 40 {
			payloadLen := int(binary.BigEndian.Uint16(decrypted[4:6]))
			totalLen := payloadLen + 40
			if totalLen > 0 && totalLen <= len(decrypted) { realPacket = decrypted[:totalLen] }
		}
	}

	srcIP, _ := GetSrcIP(realPacket)
	dstIP, _ := GetDstIP(realPacket)

	if len(realPacket) >= 40 && (realPacket[0]>>4) == 6 {
		if !strings.HasPrefix(srcIP, "fe80:") && !e.router.HasRoute(net.ParseIP(srcIP)) {
			fmt.Printf("Learned IPv6 route: %s for session %d\n", srcIP, session.ID)
	...
	isForSelf := false
	if strings.HasPrefix(dstIP, "fe80:") || dstIP == e.localVPNIP || dstIP == e.localVPNIP6 { isForSelf = true
	} else if e.cfg != nil {
		cfg := e.cfg.Get()
	...
	if isForSelf {
		if e.tun != nil { e.tun.Write(realPacket) }
	} else {
		if e.cfg == nil {
			if e.tun != nil { e.tun.Write(realPacket) } 
		} else { e.handleOutbound(realPacket) }
	}
}
...
func (e *Engine) handleHandshake(raw []byte, addr *net.UDPAddr) {
	...
	for _, peer := range cfg.Peers {
		...
		if transport.VerifyHandshake(raw, ed25519.PublicKey(pubB)) {
			...
			// Strip mask from StaticIP for internal lookup
			cleanIP := peer.StaticIP
			...
			s := &transport.Session{
				...
				StaticIP: cleanIP, PublicKey: pubB,
			}
			e.sm.Add(s)
			...
			for _, p := range cfg.Peers {
				if p.PublicKey == peer.PublicKey { continue }
