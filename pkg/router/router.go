...
func (r *Router) AddSubnet(cidr string, sessionID uint64) error {
	...
	// Update StaticIP in session manager if this is the primary IP
	if session, ok := r.sm.GetByID(sessionID); ok {
		if session.StaticIP == "" {
			session.StaticIP = ipStr
			r.sm.Add(session) // Re-add to update peers map
		}
	}
